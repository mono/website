...
for how other VMs do cache flushing on ARM64 and we found out about some
...
is not enough for user space code as it can be rescheduled right after on a core with the smaller cache line size. As a workaround we detect
when there are multiple sizes and use the smaller one but even that can still fail.

Here is our fix for Mono: [Pull Request](https://github.com/mono/mono/pull/3549)



[1] [V8's cache flushing](https://github.com/v8/v8/blob/fec99c689b8587b863df4a5c4793c601772ef663/src/arm64/cpu-arm64.cc#L40)
[2] [LLVM's cache flushing](https://github.com/llvm-mirror/compiler-rt/blob/ff75f2a0260b1940436a483413091c5770427c04/lib/builtins/clear_cache.c#L146)
