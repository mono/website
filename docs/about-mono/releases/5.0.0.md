...
Roslyn C# compiler is now available on Mono and it's called `csc`. It's same compiler available on Windows with full support of C#7. Both msbuild and xbuild has been updated to call csc compiler under the hood instead of mcs.

Replacing `mcs` with `csc` in user scripts should be straightforward but small issues can arise as command line arguments accepted by csc are not identical to mcs.

Following new C#7 features are now available
- Tuples
- Pattern Matching
- Local Functions
...
Mono C# compiler is still available but it has not been updated to include C#7 features. The compiler is still maintained and all reported bugs have been fixed.
...
Mono version of msbuild replacement called xbuild is now deprecated. We encourange everyone to switch to `msbuild` which is now available on Mono as well and it's a copy of .NET version of msbuild resolving numerous incompatibilities we had in our xbuild implementation.

### Reference assemblies

Added support for targeting .NET 4.6.2. In the next release we'll be updating the reference assemblies so we're matching the APIs
