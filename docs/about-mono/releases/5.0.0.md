...
Added support for targeting .NET 4.6.2. In the next release we'll be updating the reference assemblies so we're matching the APIs that can be targeted for all .NET profiles.
...
As we [announced](http://www.mono-project.com/news/2016/11/29/mono-code-sharing/) 
we are adopting a new strategy to share code with .NET.   With this release we started
