...
The current status of ASP.NET 2.0 features is tracked [here]({{ site.github.url }}/ASPTests).
...
Details about xbuild are available [here]({{ site.github.url }}/Microsoft.Build).
...
Support for SafeHandles has been added to Mono, for full details about what is supported see [the safehandles]({{ site.github.url }}/SafeHandles) page on the Mono Web Site [Miguel].
