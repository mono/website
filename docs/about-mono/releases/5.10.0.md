...
Multiple bug fixes affecting the Basic Class Libraries were fixed.
...
While there will be no official release at this point, the adventurous ones can try our code drops directly from CI: https://jenkins.mono-project.com/job/test-mono-mainline-webassembly/
...
The recently announced .NET 4.7.1 is now supported and available as specific target via msbuild configuration.
...
Significant parts of `System.Security` assembly implementation was replaced with CoreFX implementation.

## Tools

### C# compiler

Roslyn compiler (csc) has been updated to version 2.6.0 which includes all C# 7.2 [features](https://docs.microsoft.com/en-us/dotnet/csharp/whats-new/csharp-7-2).

Mono C# compiler (mcs) gained support for following C# 7.x features
- default literal
...
Updated to the latest 4.1 version.

FSharp.Core assembly is no longer installed into GAC.

### msbuild

The msbuild was updated to the latest version available.

Mono incompatibility nuget assemblies list was extended and updated which should help with nugets which bundle same assemblies installed by Mono.
