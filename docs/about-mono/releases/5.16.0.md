...
A new option `llvmopts` has been added to allow to override the built-in set of flags passed to the LLVM optimizer.

### Response files support

A response file is a plain-text file that contains strings to be treated as arguments for mono. You can now use option `--response=FILE` to pass the file with arguments in and they will be treated as if the user had instead entered the arguments individually.

### Assembly loader

The loading of strong named and non-strong names assemblies has been improved to further match .NET desktop behaviour. At the same time, we have introduces a new separation of loader contexts which can influence the loader behaviour. The context could be seen when using Mono debug options and we recognize 4 distinguish contexts.

* default - Assembly.Load uses it
* reflection only - Assembly.ReflectionOnlyLoad and Assembly.ReflectionOnlyLoadFrom use it
* load from - Assembly.LoadFrom uses this one
* individual - Assembly.LoadFile use it
...
Performing 32-bit floating operations using 32-bit math is now the default settings for the runtime. Previously you had to enable this setting manually by using `-O=float32` option.

### Copy file on macOS

We have rewritten file copy operations on macOS to use special native API for file copying. This can significantly speed up large files operations on Apple File System (APFS).

### The debugger library

The runtime can be configured using `--enable-minimal=soft_debug` option and the debugger code will be compiled into a separate `libmono-dbg.a` library which needs to be linked into the app. 

### Custom modifiers enhancements

The custom modifiers are not broadly used a feature of IL but getting more exposure in the latest C# compilers. We have fixed a few issues in handling them especially around type equality involving custom modifiers.
...
The TLS tenegotiation on macOS requires version 10.12 or newer.

### System.MathF type

The single-precision math operations are now available in a new `System.MathF` class. Together with the `-O=float32` option, this can be used to noticeably improve the performance in floating point code.
...
The arbitrary command nesting is now supported. The two new mechanisms were introduced to allow that. The command names may now contain spaces and `CommandSets` may be nested within `CommandSets`. This allows to supporting syntax like:
