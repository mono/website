...
This documents the changes since [Mono 2.2](/docs/about-mono/releases/2.2.0/ "Release Notes Mono 2.2").
...
If only one application is configured in [mod\_mono](/docs/web/mod_mono/) or Mono's XSP Mono will eliminate the intermediate multiplexor/demultiplexor for requests and instead send and receive the data straight from one domain to the kernel.
