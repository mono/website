...
This documents the changes since [Mono 2.0]({{ site.github.url }}/docs/about-mono/releases/2.0.0/ "Release Notes Mono 2.0")
...
The code generation engine has been replaced from tree-based representation that we used for 1.0.x, 1.2.x, 1.9 and 2.0 releases to a representation that is better suited for advanced optimizations, the [Linear IL]({{ site.github.url }}/docs/advanced/runtime/docs/linear-ir/) engine.
...
Support for [Full Ahead of Time Compilation]({{ site.github.url }}/docs/advanced/aot/), generic sharing, and static linking is now available.
...
A new interactive shell for C\# is available, the command is called "csharp", more details are available on the csharp manual page and on the [CsharpRepl]({{ site.github.url }}/docs/tools+libraries/tools/repl/) page.
...
The [Mono DataConvert]({{ site.github.url }}/archived/Mono_DataConvert "Mono DataConvert") library provides encoding and decoding of bytes, words, strings and other data types to and from various CPU endianness.
