...
Then in the bin directory of your mono installation folder (MONO_INSTALLATION_PATH\bin\) you should see some of these libraries: libmonoboehm-2.0.dll, libmonosgen-2.0.dll, libmono-2.0.dll or mono-2.0.dll. Depending on your mono's version you can see different names (in the current versions you will find libmonoboehm-2.0.dll and [libmonosgen-2.0.dll](/docs/advanced/garbage-collector/sgen/)) you must choose one of them and edit the mono.def file. For example, if you choose libmonosgen-2.0.dll the mono.def file should end up like this:
