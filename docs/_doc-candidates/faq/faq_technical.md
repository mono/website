...
The documentation that used to be available on the mono/docs directory is now being moved into this web site and is available in the [Runtime Documentation]({{site.github.url}}/old_site/Mono:Runtime:Documentation "Mono:Runtime:Documentation") section.
...
Mono runs on Linux, UNIX and Windows systems. For a more detailed list, see the [Supported Platforms]({{site.github.url}}/old_site/Supported_Platforms "Supported Platforms") page.
...
This is not a perfect solution, as some APIs in Mono throw NotImplementedExceptions in certain cases, so you still should test your application with Mono.　If you care about application portability, check [MoMA]({{site.github.url}}/old_site/MoMA "MoMA"), the migration analyzer.
...
See our [Supported Platforms]({{site.github.url}}/old_site/Supported_Platforms "Supported Platforms") page.
...
You can find which native methods an assembly is using by using [MoMA]({{site.github.url}}/old_site/MoMA "MoMA").
...
[Gendarme]({{site.github.url}}/old_site/Gendarme "Gendarme") also gives you some interoperability warnings.

**Must I have mono to create or run Gtk\# applications in Windows?**

No. Currently you can use the [Gtk-Sharp Installer for .NET Framework]({{site.github.url}}/old_site/Gtk-Sharp_Installer_for_.NET_Framework "Gtk-Sharp Installer for .NET Framework") which when coupled with the Microsoft .NET Framework, will allow you to build .NET applications that use Gtk\# as their graphical user interface.
...
For more details see our [C++ page]({{site.github.url}}/old_site/CPlusPlus "CPlusPlus").

**Does Mono have a Global Assembly Cache (GAC)?**

Yes, Mono has a Global Assembly Cache.

It operates in a similar way to the .NET Global Assembly Cache, but has a few extensions, see the [Assemblies and the GAC]({{site.github.url}}/old_site/Assemblies_and_the_GAC "Assemblies and the GAC") article for details.
...
See the [Config]({{site.github.url}}/old_site/Config "Config") page, and in particular the [\<dllmap\>]({{site.github.url}}/old_site/Config_DllMap "Config DllMap") and [\<dllentry\>]({{site.github.url}}/old_site/Config_DllMap "Config DllMap") directives.
...
Yes, see our [Debugger]({{site.github.url}}/old_site/Debugger "Debugger") page for more information.
...
In addition it is possible to create custom profilers using Mono's profiling interface. For a list of existing profilers for Mono see our [Performance Tips]({{site.github.url}}/old_site/Performance_Tips "Performance Tips") page.

A more advanced profiler is the [Logging Profiler]({{site.github.url}}/old_site/Profiler)

**Does Mono have a memory profiler?**

Mono has several memory profilers: a built-in one (see manual page), [HeapShot]({{site.github.url}}/old_site/HeapShot "HeapShot") for information about the Mono live memory usage, [HeapBuddy]({{site.github.url}}/old_site/HeapBuddy "HeapBuddy") for memory allocation patterns and [OprofileWithAnonJitData using OProfile to profile JITed code](/index.php?title=OprofileWithAnonJitData_using_OProfile_to_profile_JITed_code&action=edit&redlink=1 "OprofileWithAnonJitData using OProfile to profile JITed code (page does not exist)").

See the [Profile]({{site.github.url}}/old_site/Profile "Profile") page for more information.
...
See the [Code Coverage]({{site.github.url}}/old_site/Code_Coverage "Code Coverage") page for more information.
...
3.  [WCF]({{site.github.url}}/old_site/WCF_Development), preliminary
...
More information about the Mono documentation can be found on the [Documentation]({{site.github.url}}/old_site/Documentation "Documentation") page.
...
If you want to merge the assemblies, you could try the prototype "merge" tool that is part of [Cecil]({{site.github.url}}/old_site/Cecil "Cecil").
...
**How do I build mono/mcs on Windows from Mono's Subversion [repository]({{site.github.url}}/old_site/SourceCodeRepository)?**

See this "[Compiling Mono on Windows]({{site.github.url}}/old_site/Compiling_Mono_on_Windows "Compiling Mono on Windows")" page.
...
The ADO.NET support is fairly complete and there are many [database providers]({{site.github.url}}/old_site/Database_Access "Database Access") available.
...
Use Mono.Data.SqliteClient. Make sure you are using at least Mono 1.1.4 since SQL Lite provider does not work in the Mono 1.0.x releases and prior. Use a connection string like "URI=file:SqliteTest.db" if you are using SQL Lite version 2.x or use "version=3,URI=file:SqliteTest.db" if using SQL Lite version 3.x. See [SQLite]({{site.github.url}}/old_site/SQLite "SQLite") for more info.
...
Yes. The Microsoft class collection is very big, but it is by no means complete. For more information on extending Mono, see our [ideas]({{site.github.url}}/old_site/Ideas "Ideas") page.
...
We recommend that the libraries are given a chance to evolve, and have their API mature before they are installed in the GAC, which is the first step towards being considered for inclusion with Mono. To learn more about this read our [Application Deployment Guidelines](/index.php?title=Application_Deployment_Guidelines&action=edit&redlink=1 "Application Deployment Guidelines (page does not exist)").
...
Mono currently runs on [Linux]({{site.github.url}}/old_site/Mono:Linux "Mono:Linux"), [Windows]({{site.github.url}}/old_site/Using_Mono_on_Windows), Solaris, [FreeBSD]({{site.github.url}}/old_site/Mono:BSD "Mono:BSD") and [MacOS X]({{site.github.url}}/old_site/Mono:OSX "Mono:OSX"). The Just-In-Time engine (JIT) is available on [x86]({{site.github.url}}/old_site/Mono:X86 "Mono:X86") and [PowerPC]({{site.github.url}}/old_site/Mono:PowerPC "Mono:PowerPC"), [Sparc]({{site.github.url}}/old_site/Mono:SPARC "Mono:SPARC") and S390 processors and can generate code and optimizations tailored for a particular CPU. Interpreters exist for the Itanium, HP-PA, StrongARM CPUs.
...
An implementation of the VB.NET language is now available, to find out more about the current state of the compiler see the page on [Basic]({{site.github.url}}/old_site/VisualBasic.NET_support).
...
-   the quality of the generated code, this being a metric of the code generator and the optimizations implemented on it. You can read about some optimizations in Mono on the [Runtime](/index.php?title=Runtime&action=edit&redlink=1 "Runtime (page does not exist)") page.
...
We are currently working on a [Generational GC]({{site.github.url}}/old_site/Generational_GC "Generational GC") that will assist in some of the limitations described above.
...
Read the [Runtime](/index.php?title=Runtime&action=edit&redlink=1 "Runtime (page does not exist)") page for more details.
...
We have a number of recommendations in our [Performance Tips]({{site.github.url}}/old_site/Performance_Tips "Performance Tips") page covering some common practices to improve the performance of your application.
...
-   Mono has a complete [C\# 1.0](/index.php?title=Mono_C&action=edit&redlink=1 "Mono C (page does not exist)") implementation and has been stress tested a lot more than Portable.NET's compiler.
...
-   Mono has a [complete managed XML stack]({{site.github.url}}/old_site/XML): XML, XPath, XML Serializer, XML Schema handling are fully functional, feature complete and tuned for performance. In addition to the Microsoft API, mono ships with the Mono.Xml.Ext library that contains an XQuery/XPath2 implementations.
